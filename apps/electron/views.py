...
class ElectronViewSet(generics.RetrieveUpdateAPIView, generics.ListAPIView, generics.DestroyAPIView, viewsets.GenericViewSet):
    ...
    def schemes(self, request, *args, **kwargs):
        ...
        try:
            electron = self.get_object()
            scheme_electrons = SchemeElectron.objects.filter(model_name=electron.model_name, scheme__is_reference=True)
            queryset = [scheme_electron.scheme for scheme_electron in scheme_electrons]
            page = self.paginate_queryset(queryset)
            serializer = self.get_serializer(page, many=True)
            return self.get_paginated_response(serializer.data)
...
#     """
#     queryset = Electron.objects.all()
#     serializer_class = ElectronDataSheetSerializer
#
...
# # 元器件图片
# class ElectronImageViewSet(generics.RetrieveUpdateAPIView, generics.DestroyAPIView, viewsets.GenericViewSet):
#     """
#         delete:
#            元器件Image文件删除
#         update:
...
#     """
#     queryset = Electron.objects.all()
#     serializer_class = ElectronUploadImageSerializer
#
#     def perform_destroy(self, instance):
#         instance.images = ""
#         instance.save()
...
class ElectronVideoViewSet(viewsets.GenericViewSet):
    ...
    # @action(['get'], detail=False)
    # def electron_list(self, request):
    #     """元器件对应的视频"""
    #     try:
    #         electron_id = request.query_parmas.get('id')
    #         electron = Electron.objects.get(id=electron_id)
    #         electron_videos = ElectronVideo.objects.filter(electron=electron)
    #         serializer = ElectronVideoSerializer(electron_videos, many=True)
    #         return Response(serializer.data, status=status.HTTP_200_OK)
    #     except Electron.DoesNotExist:
    #         return Response({'message': '对象不存在'}, status=status.HTTP_400_BAD_REQUEST)
    @action(['get'], detail=True)
...
# # 通用的视频类
# class UniversalVideoViewSet(viewsets.ModelViewSet):
#     queryset = UniversalVideo.objects.all()
#     serializer_class = UniversalVideoSerializer
